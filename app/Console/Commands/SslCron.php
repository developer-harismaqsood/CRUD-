...
class SslCron extends Command
{
    ...
    {
        $order  =  User::where('status','domain')->first();
        $Server =  Server::first();
          ...
          $fields =  array(
                      ...
                      'ssl_domains' => array($filter_url),
                      'wild_card' => false,
                      'ssl_email' => $order->email
                   );

          $data = json_encode($fields);   

          $url = 'security/lets_encrypt_install'; $type = 'post'; $access_token = CronsHelper::getAccessToken($Server->server_id);
